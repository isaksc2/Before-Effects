        ...
        return (
            <div>
                <TextField 
                    ...
                    value = {this.state.document && !this.state.editingUsername ? this.state.document.username : this.state.tempUsername}
                    label = "Username">
                </TextField>
                <Button hidden={!(this.props.uid === authentication.currentUser.uid)} onClick={this.editUsername}>{this.editUsernameIcon(this.state.editingUsername)}</Button>
                <div>
                    {
                        this.state.document && this.state.document.posts ? 
                        this.state.document.posts.split("¤").map(post => post.split(";")).map(post => (
                        <Thumbnail vID1={post[0]} vID2={post[1]} title={post[2]}></Thumbnail>
                        ))
                        :
                        <h1>{(this.props.uid === authentication.currentUser.uid) ? "You have not posted any videos yet" : "This user has not posted any video yet"}</h1>
                    }
                </div>
            </div>
        )
